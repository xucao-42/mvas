...
class RayTracing(nn.Module):
    ...
    def forward(self,
        ...
        sphere_intersections, mask_intersect = get_sphere_intersection(cam_loc, ray_directions,
                                                                       r=self.object_bounding_sphere)  # (num_pixel, 2) (num_pixel, )

        curr_start_points, unfinished_mask_start, acc_start_dis, acc_end_dis, min_dis, max_dis = \
            self.sphere_tracing(sdf, cam_loc, ray_directions, mask_intersect, sphere_intersections)
        ...
        if sampler_mask.sum() > 0:
            sampler_min_max = torch.zeros((batch_size, num_pixels, 2)).to(device)
        ...
        in_mask = ~network_object_mask & object_mask & ~sampler_mask  # no intesection and inside gt mask and
        ...
        if mask_left_out.sum() > 0:  # project the origin to the not intersect points on the sphere
            ...
            acc_start_dis[mask_left_out] = -torch.bmm(rays_left_out.view(-1, 1, 3),
                                                      cam_left_out.view(-1, 3, 1)).squeeze()
        ...
        if mask.sum() > 0:
            ...
            min_mask_points, min_mask_dist = self.minimal_sdf_points(num_pixels, sdf, cam_loc, ray_directions, mask,
                                                                     min_dis, max_dis)
    ...
    def sphere_tracing(self, sdf, cam_loc, ray_directions, mask_intersect, sphere_intersections):
        ''' Run sphere tracing algorithm for max iterations from both sides of unit sphere intersection '''

        num_pixels = ray_directions.shape[0]
        sphere_intersections_points = cam_loc.reshape(num_pixels, 1, 3) + sphere_intersections.reshape(num_pixels, 2,
                                                                                                       1) * ray_directions.reshape(
            num_pixels, 1, 3)  # (num_pixel, 2, 3)
        unfinished_mask_start = mask_intersect.clone()
        unfinished_mask_end = mask_intersect.clone()

        # Initialize start current points
        curr_start_points = torch.zeros(num_pixels, 3).to(device).float()
        curr_start_points[unfinished_mask_start] = sphere_intersections_points[:, 0, :][
            unfinished_mask_start]  # (num_pixel, 3)
        acc_start_dis = torch.zeros(num_pixels).to(device).float()
        acc_start_dis[unfinished_mask_start] = sphere_intersections[unfinished_mask_start, 0]  # (num_pixel,)

        # Initialize end current points
        curr_end_points = torch.zeros(num_pixels, 3).to(device).float()
        curr_end_points[unfinished_mask_end] = sphere_intersections_points[:, 1, :][
            unfinished_mask_end]  # (num_pixel, 3)
        acc_end_dis = torch.zeros(num_pixels).to(device).float()
        acc_end_dis[unfinished_mask_end] = sphere_intersections[unfinished_mask_end, 1]  # (num_pixel,)
        ...
        next_sdf_start = torch.zeros_like(acc_start_dis).to(device)  # (num_pixel,)
        next_sdf_start[unfinished_mask_start] = sdf(curr_start_points[unfinished_mask_start])

        next_sdf_end = torch.zeros_like(acc_end_dis).to(device)  # (num_pixel,)
        ...
        while True:
            ...
            # Update points
            curr_start_points = cam_loc + acc_start_dis.reshape(num_pixels, 1) * ray_directions
            curr_end_points = cam_loc + acc_end_dis.reshape(num_pixels, 1) * ray_directions
            ...
            while (
                ...
                (cam_loc + acc_start_dis.reshape(num_pixels, 1) * ray_directions)[not_projected_start]

                acc_end_dis[not_projected_end] += ((1 - self.line_search_step) / (2 ** not_proj_iters)) * curr_sdf_end[
                    not_projected_end]
                curr_end_points[not_projected_end] = (cam_loc + acc_end_dis.reshape(num_pixels, 1) * ray_directions)[
                    not_projected_end]
    ...
    def ray_sampler(self, sdf, cam_loc, object_mask, ray_directions, sampler_min_max, sampler_mask):
        ...
        sampler_pts = torch.zeros(n_total_pxl, 3).to(device).float()
        sampler_dists = torch.zeros(n_total_pxl).to(device).float()

        intervals_dist = torch.linspace(0, 1, steps=self.n_steps).to(device).view(1, 1, -1)

        pts_intervals = sampler_min_max[:, :, 0].unsqueeze(-1) + intervals_dist * (
                    sampler_min_max[:, :, 1] - sampler_min_max[:, :, 0]).unsqueeze(-1)
        points = cam_loc.unsqueeze(1) + pts_intervals[0].unsqueeze(-1) * ray_directions.unsqueeze(1)
        ...
        if n_secant_pts > 0:
            ...
            cam_loc_secant = cam_loc[mask_intersect_idx[secant_pts]]
            ray_directions_secant = ray_directions[mask_intersect_idx[secant_pts]]
